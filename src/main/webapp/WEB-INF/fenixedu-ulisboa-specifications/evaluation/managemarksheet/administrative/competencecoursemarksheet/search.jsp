...
<div class="page-header">
	<h1>
		<spring:message code="label.evaluation.manageMarkSheet.administrative.searchCompetenceCourseMarkSheet" />
		<small></small>
	</h1>
</div>
<%-- NAVIGATION --%>
	...
	<form method="get" class="form-horizontal">
		<div class="panel-body">
			<div class="form-group row">
				...
				<div class="col-sm-10">
					<input id="competenceCourseMarkSheet_executionSemester" class="form-control" type="text" name="executionsemester"
						value='<c:out value='${not empty param.executionsemester ? param.executionsemester : competenceCourseMarkSheet.executionSemester }'/>'
						required />
				</div>
			</div>
			<div class="form-group row">
				...
				<div class="col-sm-10">
					<input id="competenceCourseMarkSheet_competenceCourse" class="form-control" type="text" name="competencecourse"
						value='<c:out value='${not empty param.competencecourse ? param.competencecourse : competenceCourseMarkSheet.competenceCourse }'/>'
						required />
				</div>
			</div>
		</div>
...
" <a  class=\"btn btn-default btn-xs\" href=\"${pageContext.request.contextPath}<%=CompetenceCourseMarkSheetController.SEARCH_TO_VIEW_ACTION_URL%>{searchResult.externalId}\"><spring:message code='label.view'/></a>" +
		...
		"${datatablesI18NUrl}", }, "columns": [ { data: 'evaluationseason' }, { data: 'evaluationdate' }, { data: 'certifier' }, { data:
		'shifts' }, { data: 'creationdate' }, { data: 'creator' }, { data: 'state' }, { data: 'statedate' }, { data: 'printed' }, {
		data: 'rectified' }, { data: 'actions', className:'all' } ], //CHANGE_ME adjust the actions column width if needed "columnDefs":
		[ //54 { "width": "54px", "targets": 10 } ], "data" : searchcompetencecoursemarksheetDataSet, //Documentation:
		https://datatables.net/reference/option/dom //"dom": '<"col-sm-6"l><"col-sm-3"f><"col-sm-3"T>rtip', //FilterBox = YES &&
		ExportOptions = YES "dom": 'T<"clear">lrtip', //FilterBox = NO && ExportOptions = YES //"dom": '<"col-sm-6"l><"col-sm-6"f>rtip',
		//FilterBox = YES && ExportOptions = NO //"dom": '<"col-sm-6"l>rtip', // FilterBox = NO && ExportOptions = NO "tableTools": {
